...
def run_bc(env):
    """ Behaviour cloning experiments according to Sections 1_2 and 1_3 of assignment.pdf """

    hparams = OrderedDict()
    hparams['train_steps_per_iter'] = list(range(1000, 5000 + 1, 1000))
    hparams['lr_rate'] = list(1e1 ** np.array(range(-4, -1, 1)))

    cmd = "python cds_rl/scripts/run_hw1.py --expert_policy_file cds_rl/policies/experts/{policy}.pkl \
           --env_name {env} --exp_name {exp} --n_iter 1 \
           --expert_data cds_rl/expert_data/expert_data_{data}.pkl\
    ...
    for seed in seeds:
        for train_steps in hparams['train_steps_per_iter']:
            new_cmd = cmd.format(env=env, exp='bc_' + 'train_steps_per_iter' + str(train_steps) + '_seed' + str(seed),
                                 seed=seed,
...
def run_dagger(env):
    """ Dagger experiments according to Section 2 of assignment.pdf """

    cmd = "python cds_rl/scripts/run_hw1.py --expert_policy_file cds_rl/policies/experts/{policy}.pkl \
          --env_name {env} --exp_name {exp} --n_iter {n_iter} \
          --do_dagger --expert_data cds_rl/expert_data/expert_data_{data}.pkl \
